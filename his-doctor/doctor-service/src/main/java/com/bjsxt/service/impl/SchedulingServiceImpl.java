...
public class SchedulingServiceImpl implements SchedulingService {
    ...
    public int saveScheduling(SchedulingFormDto schedulingFormDto) {
        ...
        if (null != schedulingFormDto.getData() && schedulingFormDto.getData().size() > 0) {
            ...
            Long outpatientId = schedulingData.getOutpatientId();
            if (null != userId) {
                //删除原来这个用户的当前周的所有排班数据
                QueryWrapper<Scheduling> qw = new QueryWrapper<>();
...
//                初始化当前周 周一到周日的日期数据
                ...
                for (SchedulingFormDto.SchedulingData datump : schedulingFormDto.getData()) {
                    ...
                    for (String s : datump.getSchedulingType()) {
                        if (StringUtils.isNotBlank(s)) {
                            scheduling = new Scheduling(userId, deptId, outpatientId, schedulingDays.get(i), datump.getSubsectionType(), s, DateUtil.date(), schedulingFormDto.getSimpleUser().getUserName());
                            //保存
     ...
     */
    @Override
    public List<Long> queryHasSchedulingDeptIds(Long deptId, Long outpatientId, String schedulingDay, String schedulingType, String subsectionType) {
        return this.schedulingMapper.queryHasSchedulingDeptIds(deptId, outpatientId, schedulingDay, schedulingType, subsectionType);
    }
}
